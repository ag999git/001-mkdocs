...
|---|---|---|---|
| 1 | Introduction | One line: "...tools covered in Chapter 17." | Expanded into a full introduction explaining the standard library, why the examples matter, and how each example is arranged. Chapter number corrected from 17 to 16 to match the file name and folder |
| 2 | Table of Contents | Listed only the 8 main sections; links used the file name (`020-ch16-real-world-use.md#...`) | Rebuilt as a nested list covering every main section and sub-section, with same-page links (`#...`). Added links for the new sections |
| 3 | "Back to the Table of Contents" links | Present after only some sub-sections; the link after 7.3 pointed to a different file (`020-ch17-real-world-use.md`), which is broken | Added after every section and sub-section, all pointing to `#table-of-contents` on this page |
| 4 | New section "Modules at a Glance" | Not present | Added an overview table of all modules, tools and examples |
| 5 | Module introductions | Not present | Added a short introduction with a documentation link at the start of each of the 8 main sections |
| 6 | Sub-section headings | The sub-sections in Sections 2, 3, 5 and 6 had no numbers; in Section 3, "Student Marks List" was plain text, not a heading | Numbered as 2.1, 3.1, 5.1 and 6.1, and made "Student Marks List" a proper heading. Heading wording otherwise unchanged |
| 7 | "How it works" steps | Not present | Added numbered steps for every example |
| 8 | Scripts: step comments | No `# Step` comments | Added `# Step 1`, `# Step 2` and so on to every script. All original explanatory comments kept |
| 9 | Scripts: formatting | Several scripts had double spaces (for example `from  collections  import  namedtuple`, `Employee  =`, `tasks  = []`) and unindented dictionary contents in 1.6 | Cleaned to standard single spacing and indentation. Arrow characters in comments changed to `->` |
| 10 | Output blocks | Some outputs were in `python` fences, one in an `output` fence, and several were only given as `# Output:` comments inside the script; 4.3, 7.3 and 8.3 had no output at all | All outputs placed in `text` fenced blocks after each script. Missing outputs for 4.3, 7.3 and 8.3 added. All outputs checked by running the scripts |
| 11 | 1.1 Counter: script and output | Code kept the colon (`'404:'`), so the output showed `404:: occurred 3 times` | Added `.rstrip(':')` so the output reads `404: occurred 3 times`. Added step-wise prints, a table showing how a code is extracted from a line, a flowchart, and notes on tied counts and on 200 being a success code |
| 12 | 1.2 deque | Script and output only | Added step comments and a table showing which command is dropped at each step |
| 13 | 1.3 defaultdict | Script and output only | Added a print inside the loop, a flowchart, and a second script comparing with a normal dictionary (KeyError) |
| 14 | 1.4 OrderedDict | Explanation said a modern dict remembers order | Added "Since Python 3.7", a before-and-after table, and a note on `popitem(last=False)` |
| 15 | 1.5 namedtuple | Script printed only name and department | Added prints of the split list and the object, and a Step 6 showing values are strings and how to convert the salary with `int()`. Explanation expanded |
| 16 | 1.6 ChainMap | Explanation said "This is exactly how professional config parsers work" | Softened to a more accurate statement. Added a flowchart and a Step 7 showing that changes go only to the first dictionary |
| 17 | 2.1 heapq: explanation | Said `heappush` "automatically sorts them internally" | Corrected: a heap keeps only the smallest item at index 0 and is not fully sorted. Added prints of the heap after each push and a flowchart |
| 18 | 3.1 bisect: explanation | Referred to the value 350 and to "large leaderboards", which do not appear in the script (the script inserts 65 into a marks list) | Rewritten to match the script (65 at index 2). Corrected the claim about speed: finding the spot is fast, but list insertion still shifts items. Added a binary search trace table, a flowchart, descending-order output, and a note that the list must be sorted |
| 19 | 3.1 bisect: inline output comments | `# Output:` comments on print lines | Moved into a separate output block |
| 20 | 4.1 Queue: explanation | Said `queue.Queue` is "mandatory" with multiple threads | Reworded as "the right choice" with the reason. Added `qsize()` prints and a definition of thread and thread-safe |
| 21 | 4.2 LifoQueue | Undid only one action | Added a Step 4 undoing the remaining actions, and a table of add and undo order |
| 22 | 4.3 PriorityQueue | No comments, no output | Added comments, output, a definition of triage, and a note that it uses `heapq` inside |
| 23 | New sub-section 4.4 | Not present | Added a comparison table of queue types, `deque` and `heapq` |
| 24 | 5.1 enum: explanation and comments | Said `HttpStatus.NOT_FOUND` "evaluates to 404"; a comment said Python "compares HttpStatus.NOT_FOUND.value"; said Python will "throw an error" for a typo | Corrected: the member is not equal to 404; `.value` is 404; Python compares the members themselves; the typo raises `AttributeError`. Added a `SERVER_ERROR` branch and an `else` branch to the function, Steps 4 and 5 showing name, value, lookup and the typo error, a flowchart, and links to `IntEnum` and `http.HTTPStatus` |
| 25 | 6.1 dataclasses: comment on mutable default | Said writing `tags: list = []` means "all objects could accidentally share the same list" | Corrected: in a dataclass this raises a `ValueError`; sharing happens in a normal class. Added a table of generated methods, printing of `p2`, an equality check, and a note on `150.0` |
| 26 | 7.1 lru_cache: notes in comments | Notes said "Fetching user 42: is printed only once, because the function body is skipped" (that line is printed by the main script, not the function body) | Corrected: it is the "Querying database" line that appears only once. Removed a stray `#` in a comment. Added timing with `time.perf_counter()`, `cache_info()`, a flowchart, and a caution about data that changes. "Thousands of percent" reworded |
| 27 | 7.2 partial | Two calls shown | Added a normal call for comparison and a `log_warning` example. Comment updated to show the keyword form `level="ERROR"` |
| 28 | 7.3 reduce | No comments or output | Added comments, a trace table, a flowchart, output, a comparison with `sum()`, and a note on float rounding and the `decimal` module |
| 29 | 8.1 chain | Output given only as comments; explanation said "incredibly memory efficient ... in RAM" | Added a `print()` to end the line, a Step 3 using `chain()` on separate lists, and an output block. Wording simplified |
| 30 | 8.2 groupby | Only sorted data shown | Added Steps 3 and 4 showing the unsorted-data problem and the sorting fix |
| 31 | 8.3 product | No comments or output; explanation used LaTeX `$3 \times 3 = 9$` | Added comments, a combinations grid, an output block, a nested-loop comparison, and plain text "3 x 3 = 9" |
| 32 | 8.4 permutations: comment | Said "The number of possible arrangements is the number of items in the list" (incorrect: it is 3 factorial, not 3) | Corrected. Also changed "your table only has 3 distinct chairs" to "your table has 3 distinct chairs". Added a Step 4 with 2 chairs and a table comparing `product`, `permutations` and `combinations` |
| 33 | New section 9 | Not present | Added nine "Check Your Understanding" questions with answers |
| 34 | New section 10 | Not present | Added a summary |
