...
|---|---|---|---|
| 1 | Introduction | Page began with the title and the "Conceptual Questions with Answers" heading only | Added an introduction explaining what the page contains, how it links to Chapter 16 and to Python in general, and how to use the page |
| 2 | Table of Contents | Not present | Added a nested Table of Contents covering the main sections and all 20 questions |
| 3 | "Back to the Table of Contents" links | Not present | Added at the end of every section and sub-section |
| 4 | New section "Big-O Notation in Brief" | Not present | Added a short explanation of Big-O with two reference tables, because most answers rely on it |
| 5 | Question headings | Questions were bold lines. Some answer sub-headings used `###`, some used `####`, and some were bold text. In Q7, Q9, Q14, Q15, Q18, Q19 and Q20 the word "Answer:" or a sub-heading was merged into the question line or the "Answer" heading | Each question now has its own `###` heading with a short title, followed by the printed question text in bold. Answer sub-headings made consistent as `####`. "Answer:" removed from question lines. Question wording itself unchanged |
| 6 | Q1 question layout | (a) was a `###` heading while (b) and (c) were bold. Part (c) was written "(c )" | All three parts shown the same way as bold lines. "(c )" corrected to "(c)" |
| 7 | Q1(b) formula | "After k comparisons: (n/k²) items remain ... (n/k²) = 1, so k = log₂(n)" | Corrected to `n/2ᵏ` items remain and `n/2ᵏ = 1`, so `2ᵏ = n` and `k = log₂(n)`. Steps numbered. Added a halving table, a binary search flowchart and a script counting comparisons |
| 8 | Q1(c) | Said the list "must be re-sorted before each binary search" and suggested `insort()` "or a balanced BST" as structures that maintain order | Clarified that re-sorting is needed only when changes break the order, that `insort()` still costs `O(n)` per insert, and that Python's standard library has no balanced tree (a third-party option and `set`/`dict` mentioned). Added a decision table and explanation of BST |
| 9 | Q1 follow-up questions | Not present | Added two follow-up questions with answers |
| 10 | Q2 | Explanation only | Added a one-pass trace table, numbered derivation steps with a worked example for n = 5, a flowchart, a script comparing basic and optimised versions, and two follow-up questions |
| 11 | Q3 memory writes example | "Insertion Sort: 9 shifts × 1 write = 9 memory writes" | Corrected to 9 shifts plus 1 final placement of the key = 10 writes. Wording "roughly 3 times less writing" changed to "roughly a third of the writing". Added the point that insertion sort also stops comparing early |
| 12 | Q3 best case | Referred to the while condition as `(numbers[position] > key)` | Described as comparing the key with the item just to its left (`numbers[position - 1] > key`), matching the new script. Added steps, a flowchart, a script counting comparisons and writes, and two follow-up questions |
| 13 | Q4 how deque works | "uses a doubly-linked internal structure ... requires only updating two pointers" | Made more precise: CPython uses a doubly-linked list of blocks. Added that indexing near the middle of a deque is `O(n)` |
| 14 | Q4 `insert(0, ...)` on a deque | Said `deque.insert(0, item)` is `O(n)` and advised never to use `insert(0, ...)` or `pop(0)` on a deque | Corrected: `list.insert(0, x)` is `O(n)`; on a deque, `insert()` in the middle is `O(n)`, but CPython treats `insert(0, x)` as `appendleft()`. Added that `deque.pop()` takes no argument, so `pop(0)` raises `TypeError`. Added a comparison table, a pop(0) diagram, a timing script and two follow-up questions |
| 15 | Q5 | Explanation with an indented, partly broken bullet list | Rebuilt the `defaultdict` list as a table, added a "choosing between them" table and a script demonstrating all three tools, including ChainMap's live view and writes |
| 16 | Q6 table: default values | "Not supported directly (workaround: `__new__` override)" | Corrected: supported through the `defaults` argument since Python 3.7 |
| 17 | Q6 table: methods auto-generated | namedtuple: "`__repr__`, `__eq__` only" | Corrected: namedtuple also gets a constructor and all tuple behaviour (`__eq__`, `__hash__`, ordering, indexing), plus `_asdict()`, `_replace()` and `_make()` |
| 18 | Q6 table: other rows | Type hints and memory rows | Added mention of `typing.NamedTuple` and of `slots=True` for dataclasses |
| 19 | Q6 mutable default | "writing subjects: `list = []` in a dataclass. This creates one shared list for all instances" | Corrected: a dataclass raises `ValueError` for this. The shared-list problem applies to normal classes. Fixed the misplaced code formatting. Added a script and two follow-up questions |
| 20 | Q7 question line | Ended with "Answer:" | "Answer:" removed from the question line |
| 21 | Q7 answer | Explanation, one code block and a stray `-----` line | Added index formula table, a tree diagram of a sample heap, output block, the `(priority, count, item)` tie-breaker pattern, a note on Python 3.14 max-heap functions, a full script, and two follow-up questions. Stray line removed |
| 22 | Q8 purpose | Said both finding and inserting "are `O(log n)` rather than `O(n)`" | Corrected: finding the position is `O(log n)`, but inserting is `O(n)` because items shift. Fixed the broken formatting of `bisect\_right`. Added output blocks, a position table and a grade-lookup script |
| 23 | Q9 heading | "Answer: Thread-safety defined:" | Split into a normal answer sub-heading |
| 24 | Q9 `task_done()` | "`task_done()` must be called after processing each non-None task so that `queue.join()` can correctly detect when all work is complete" | Corrected: `task_done()` must be called for every item taken, including sentinels, if `join()` is used after they are added. Added a race condition example, a comparison table, a flowchart and a complete working worker script |
| 25 | Q10 Fibonacci call count | "`fib(32)` triggers roughly 2³² individual calls" | Corrected: `fib(32)` makes 7,049,155 calls. `O(2ⁿ)` kept as the upper bound. Fixed "`@lru\_cache`" formatting. Added a flowchart, a script with `cache_info()` results, a decorator comparison table, and cautions about hashable arguments and changing data |
| 26 | Q11 | Explanation and one code block | Added output block, step-by-step explanation, extra reasons to prefer `partial()` (inspection, early binding, pickling), and a script showing the difference from a lambda |
| 27 | Q12 table | `groupby` row said "Data MUST be sorted by key first". `islice` signature shown with start and stop only | Reworded: sort first if you want one group per key. Both `islice` signatures shown. Added counting formulas for combinations and permutations, and a script for all five functions |
| 28 | Q12 groupby gotcha | Said the group iterator "is consumed as you iterate" | Clarified that a group becomes empty once `groupby()` moves to the next group. Added a script showing both problems |
| 29 | Q13 `move_to_end` | "you must manually pop the key and re-insert it, which is three operations" | Corrected to two steps (`d[key] = d.pop(key)`), and added that moving to the beginning needs rebuilding the dictionary |
| 30 | Q13 `popitem` | "Plain dict's `popitem()` always removes an arbitrary item in Python 3.7 and the last item from Python 3.8" | Corrected: plain `dict.popitem()` is guaranteed to remove the last item from Python 3.7 onward; before 3.7 it was arbitrary |
| 31 | Q13 equality | Explanation only | Added a table, including `OrderedDict == dict`, and a demonstration script |
| 32 | Q14 heading | "Answer: Magic numbers:" | Split into a normal answer sub-heading |
| 33 | Q14 `auto()` | Said `auto()` "prevents accidental duplicate values". `auto` was imported but not used in the example | Explained aliases and the `@unique` decorator. Added a script using `auto()`, iteration, comparisons, lookups and the reassignment error. Added a comparison table |
| 34 | Q15 question line | Ended with "Answer:" | "Answer:" removed from the question line |
| 35 | Q15 | Signature shown as `reduce(function, iterable)` | Added the optional initializer and the empty-list error. Added the year and title of Guido van Rossum's article, a trace table, a flowchart, mention of `sum()`, `max()` and `math.prod()`, and a script. "not naturally associative" advice reworded in simpler terms |
| 36 | Q15 separator | Stray `-----` line before Q16 | Removed |
| 37 | Q16 table | Table rows were broken across several lines, with text split into empty extra rows | Rebuilt as a proper table. Added that tuples become lists and keys become strings in json |
| 38 | Q16 security and practical rule | Explanation only | Explained serialisation in plain words, why loading pickle can run code, and why json does not. Added a decision flowchart and a script comparing json and pickle |
| 39 | Q17 | Explanation only | Added a pass-by-pass table, a flowchart, a comparison table of the three sorts, a note that selection sort is not stable, and a script counting comparisons and swaps |
| 40 | Q18 headings | "Answer: Factory function:" | Split into a normal answer sub-heading |
| 41 | Q18 `__mro__` output | Output shown inside inline code with a stray backtick placement | Shown as a proper text block with steps explaining each part |
| 42 | Q18 `_asdict()` example | Code block without a language label, and `Student` used without being defined | Made into a complete runnable script with an output block. Added the pickle problem caused by mismatched names, and a complete script covering all four ideas |
| 43 | Q19 heading | "Answer: `perf_counter()` vs `time.time()`:" | Split into a normal answer sub-heading |
| 44 | Q19 `time.time()` | Said it can be affected by "daylight saving changes" | Corrected: it is based on UTC and not affected by daylight saving, but is affected by system clock changes such as NTP adjustments. Added a comparison table and a timing script |
| 45 | Q19 graph shapes | Table in order O(n), O(log n), O(n²), O(n log n) | Reordered from slowest to fastest growth and wording simplified. Added an image of the four growth curves (`resources/060-ch16-complexity-growth.png`) and the script that draws it |
| 46 | Q20 headings | "Answer: Batteries included:" | Split into a normal answer sub-heading |
| 47 | Q20 batteries included | Listed "cryptography" as covered by the Standard Library | Changed to "hashing and secure random numbers", since full cryptography needs third-party packages. Added an explanation of API |
| 48 | Q20 how importing works | "modules - individual Python files" | Clarified that modules can also be packages or built into the interpreter. Added the steps Python follows on import, a flowchart, `import module as alias`, an explanation of namespace, and a script including a real `from os import *` example |
| 49 | General formatting | Stray escape characters (`pass\_no`, `cache\_info()`), inconsistent code formatting, and extra blank table lines | Cleaned up throughout |
| 50 | Outputs | Most code blocks had outputs only as inline comments, or none | Every runnable script now has an output block produced by actually running it |
