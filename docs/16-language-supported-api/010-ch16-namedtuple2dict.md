...
| No. | Element | Original file | Change made (added / modified / deleted) |
|---|---|---|---|
| 1 | Page title | Began with the exercise heading "Exercise: Convert a NamedTuple into a Dictionary" | Added a main title "Converting a NamedTuple into a Dictionary" and an introduction explaining what the page covers and how it links to Python and to Chapter 16 |
| 2 | Table of Contents | Not present | Added a nested Table of Contents with links to all main headers and sub-headers |
| 3 | "Back to the Table of Contents" links | Not present | Added at the end of every section and sub-section |
| 4 | Background section | Not present | Added Section 1 explaining namedtuple, dictionary, why the conversion is useful, and the tools `_fields`, `zip()` and `dict()`, with short examples, outputs and links to the Python documentation |
| 5 | Comparison table | Not present | Added a table comparing namedtuple and dictionary |
| 6 | Exercise heading | `## Exercise: Convert a NamedTuple into a Dictionary` | Numbered as "2. Exercise: Convert a NamedTuple into a Dictionary" to fit the page structure. Exercise text kept as in the original |
| 7 | Expected output in the exercise | `{ 'name': 'Anita', 'age': 20, 'course': 'Python'}` (extra space after the opening brace) | Removed the extra space so it matches what Python actually prints |
| 8 | Layout of "To create a dictionary whose keys..." line | Placed directly after the closing code fence with no blank line | Added a blank line before it so it renders correctly on GitHub. Wording unchanged |
| 9 | Horizontal rules | Written as `----------` | Replaced with `---`, and added between major sections |
| 10 | Solution heading | `## Solution Code` | Changed to "3. Solution", with "Solution code" kept as sub-section 3.4 |
| 11 | Explanation of logic | Not present | Added Section 3.1 with the steps in plain words and a table showing how the data changes at each stage |
| 12 | Flowchart for the main solution | Not present | Added a numbered Mermaid flowchart (Section 3.2) |
| 13 | Step-by-step script | Not present | Added Section 3.3 with six steps, each with its own code, print statements, output and explanation |
| 14 | Comments in the solution code | Comments were present without step numbers | Added "# Step 1" to "# Step 5" comments and two extra comment lines explaining what `zip()` and `dict()` do. Original comments kept |
| 15 | Output of the solution code | `{'name': 'Anita','age': 20,'course': 'Python'}` (missing spaces after commas), shown in a fence with no language label | Corrected to `{'name': 'Anita', 'age': 20, 'course': 'Python'}`, which is the real Python output, and placed in a `text` fenced block |
| 16 | Combined script | Not present | Added Section 3.5, one complete script combining all steps, with a docstring and step-wise printed output |
| 17 | Expected output in the extension task | Printed across two lines with inconsistent spacing: `[ {'name':'Anita','age':20,'course':'Python'},` / `{'name':'Rahul','age':22,'course':'Java'}]` | Rewritten as one line with the spacing Python actually uses |
| 18 | Closing remark "This fits well after `_fields`..." | Placed at the very end of the file, after the extension task | Moved into the extension task section, reworded slightly, and expanded to explain why no keys need to be typed |
| 19 | Solution to extension task | Not present | Added approach steps (4.1), a numbered Mermaid flowchart with a loop (4.2), a for loop solution with outputs (4.3) and a list comprehension solution with output (4.4) |
| 20 | `_asdict()` method | Not present | Added Section 5 explaining the built-in shortcut, a version note, and a comparison table |
| 21 | Follow-up questions | Not present | Added six follow-up questions with step-wise answers, code and outputs (Section 6) |
| 22 | Common mistakes | Not present | Added Section 7 as a table of common errors and their fixes |
| 23 | Summary | Not present | Added Section 8 with the key points |
