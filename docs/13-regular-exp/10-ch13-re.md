...
> **Structural fix applied here:** in the original page, the explanatory sentence "*To check whether a specific flag is set…*" and a second `import re` had accidentally been left **inside** the first code block, which would make the block fail to run as Python at all (plain English text is not valid Python syntax). The example is now split into the two separate, independently runnable scripts shown above.
...
> **Fix applied here:** the original text claimed that using a capturing group here would make `findall()` return a list of one-element **tuples**, like `[('cat',), ('dog',), ('rat',)]`. That is only true when a pattern has **two or more** capturing groups. Python's own documentation for [`re.findall()`](https://docs.python.org/3/library/re.html#re.findall) is explicit about this: *"If there is exactly one group, return a list of strings matching that group."* Because `(cat|dog|rat)` has exactly **one** group, `findall()` correctly returns a plain list of strings — identical, in this particular case, to the non-capturing version. The very next script (12) shows a case with two groups, where the tuple behaviour genuinely does appear.
...
> **Fix applied here:** the original comments claimed the answers would be `docx` (for `m1.group(1)`) and `notes` (for `m2.group(1)` / `m3.group(1)`), as if the search had matched `notes.docx`. But `re.search()` always returns the *first* match it finds while scanning left to right, and `report.pdf` appears **before** `notes.docx` in the sentence — so the true, verified answers are `pdf` and `report`, as shown above. If you genuinely wanted to reach `notes.docx` specifically, you would need `re.findall()` (to get every match) or a more specific pattern that rules out `report.pdf`.
...
```mermaid
flowchart TD
    A["Start: number n"] --> B["Build unary string: '1' repeated n times"]
    B --> C{"Does the regex\n^1?$ or ^(11+?)\1+$\nmatch the whole string?"}
    C -- "Yes, it matches" --> D["n is 0, 1, or composite\nreturn False (not prime)"]
    C -- "No match found" --> E["No repeating block exists\nreturn True (prime)"]
```
...
```mermaid
flowchart TD
    A["Start: read the whole password"] --> B{"Contains a lowercase letter?"}
    B -- No --> Z["Reject: BAD"]
    B -- Yes --> C{"Contains an uppercase letter?"}
    C -- No --> Z
    C -- Yes --> D{"Contains a digit?"}
    D -- No --> Z
    D -- Yes --> E{"Contains an allowed special\ncharacter from !@#$%^&*?"}
    E -- No --> Z
    E -- Yes --> F{"Length between 6 and 12,\nusing only allowed characters?"}
    F -- No --> Z
    F -- Yes --> G["Accept: GOOD"]
```
...
```mermaid
flowchart LR
    S["snake_case"] -- snake_to_camel --> C["camelCase"]
    C -- camel_to_snake --> S
    S -- snake_to_pascal --> P["PascalCase"]
    P -- pascal_to_snake --> S
    C -- camel_to_pascal --> P
    P -- pascal_to_camel --> C
    S -- snake_to_upper --> U["UPPER_CASE"]
    U -- upper_to_snake --> S
    U -- upper_to_camel --> C
    C -- camel_to_upper --> U
    U -- upper_to_pascal --> P
...
*(The extra `camel_to_snake("XMLFile")` demonstration line has been added here to verify, with real output, the exact "XMLFile -> XML_File" example promised in that function's own docstring — and it is confirmed correct: `xml_file`.)*
...
```mermaid
flowchart TB
    subgraph DFA["DFA (text-directed)"]
        direction TB
        D1["Read next character"] --> D2["Move to the ONE next state\nthat character allows"]
        D2 --> D3{"End of text?"}
        D3 -- No --> D1
        D3 -- Yes --> D4["Accept or reject\n(no backtracking, ever)"]
    end
    subgraph NFA["NFA (regex-directed, e.g. Python re)"]
        direction TB
        N1["Try one possible path\nthrough the pattern"] --> N2{"Does the rest\nof the pattern\nsucceed from here?"}
        N2 -- Yes --> N3["Accept"]
        N2 -- No --> N4["Backtrack: undo the\nlast choice and try\nthe next possibility"]
        N4 --> N1
        N4 --> N5["No possibilities left:\nreject"]
    end
```
...
The same flow is redrawn below as a diagram you can render directly on this page (and, if you like, open in a diagramming tool such as [draw.io](https://app.diagrams.net/), which can import Mermaid flowcharts like this one):

```mermaid
flowchart TD
    N1(["1. Start"]) --> N2["2. Initialize:<br/>index = 0, length = len(text)"]
    N2 --> N3{"3. text[index] == 'a' ?"}
    N3 -- Yes --> N4["4. Consume 'a':<br/>index += 1"]
    N4 --> N3
    N3 -- No --> N5{"5. text[index] == 'b' ?"}
    N5 -- No --> N9(["9. Return False"])
    N5 -- Yes --> N6["6. Consume 'b':<br/>index += 1"]
    N6 --> N7{"7. index == length ?"}
    N7 -- Yes --> N8(["8. Return True"])
    N7 -- No --> N9
```
...
```mermaid
flowchart LR
    T1["Task 1<br/>Record a trace list<br/>(what + where)"] --> T2["Task 2<br/>Return (result, count, trace)<br/>as a tuple"]
    T2 --> T3["Task 3<br/>Print the trace neatly<br/>in the test runner"]
    T3 --> T4["Task 4<br/>Explain WHY partial<br/>progress still fails"]
    T4 --> T5["Optional Challenge<br/>Add failure reasons +<br/>a visual '^' pointer"]
```
...
This page is a substantial rewrite of the original GitHub file, done in response to a specific set of instructions: improve the writing and explanations; keep every printed research/assignment question unchanged; use plain language with technical terms explained or linked; give step-by-step, logically ordered answers; comment scripts step by step; show verified output for every script; add tables and Mermaid diagrams where they help; give a combined script after any step-by-step script; and never shorten a clear, correct explanation just to save space. The table below documents what changed, organised by section, with a separate table underneath specifically listing every factual **correction** — genuine errors found in the original text, each one confirmed by actually running the code before it was corrected here.

Every script on this page (all eighteen "Scripts from the book," every Q&A example, every practice question, the primality checker, the password validator, all twelve naming-style converters, both regex-engine mini-projects, and every extension task) was re-run on Python 3.12 while preparing this page, and its output was checked against what is printed here.

#### Section-by-section summary

| Section | What was in the original | What changed here |
|---|---|---|
| Page opening | Cover image, badges, and a table of contents with no introduction | **Added:** a new title, a "What this page contains" introduction explaining the page's purpose and its relation to the printed book, a short glossary table of regex terms with links, and a "how this page is organised" outline. **Modified:** the table of contents gained an "Introduction" entry, a "Summary of changes" entry, and two corrected internal links (see the corrections table below). |
| Scripts from book (18 scripts) | Runnable code with inline `#` comments and a mix of trailing-comment and separate-comment output notation | **Modified:** every script now has `# Step N` comments explaining each line's purpose, plus its own fenced text-output block (rather than output only as trailing `#` comments). **Added:** a one-line "What this shows" summary above each script, and a version note about f-strings containing backslashes (Python 3.12+ only). **Fixed:** three genuine bugs — see the corrections table. |
| Q&A (Beginner / Intermediate / Advanced, Q1–Q21) | Bullet-point answers, a few small code snippets, some with formatting damage (statements run together on one line) | **Modified:** every answer expanded into fuller prose, every code snippet turned into a complete, runnable, commented script with verified output; two entries gained small reference tables (Q14, Q16). **Added:** two "extra practice question" call-outs (Q1, Q16), and a short demonstration of `re.escape()` under Q7. **Fixed:** two genuine bugs — see the corrections table (Q1's and Q10's and Q18's squashed one-liners were also split back into proper multi-line code; Q20's example was rebuilt as genuinely multi-line VERBOSE regex). |
| Six practice questions (Q1–Q6) | Solutions given as fragmented snippets under sub-headings, mixed with prose | **Modified:** each solution reassembled into one clean, combined, step-commented script with its own verified output block; no wording of the task/hint text was changed. |
| Primality mini-project | The book's original script, a "detailed" version, an empty placeholder `<details>` block, and the assignment/hints text | **Added:** a plain-language "core idea in four steps" walkthrough, a table explaining each piece of the regex, a Mermaid flowchart of the algorithm, a wider verified test sweep (0–23, including both edge cases), and an extra follow-up question (with real, measured timings) comparing the greedy and non-greedy versions of the pattern. **Deleted:** the empty, content-free `<details>` stub that had no body. **Kept verbatim:** the objective, restrictions, hints, suggested test cases, and deliverables exactly as printed. |
| Password validation | One script, one output block, a line-by-line explanation | **Added:** a rules-to-regex reference table, a Mermaid flowchart of the five checks, and an extra practice question about widening the special-character set. **Fixed:** one mislabelled test case — see the corrections table. **Kept:** the problem statement, the script's logic, and the full test list unchanged. |
| Naming styles (12 converters) | One long script covering all 12 conversions, no framing text | **Added:** a styles-reference table, a Mermaid diagram mapping all 12 conversion functions to each other, `# Step` comments throughout, and one extra verification line (`camel_to_snake("XMLFile")`) to confirm the example already written in that function's own docstring. No logic was changed — all 12 functions were verified to already produce the documented output. |
| DFA / NFA discussion | Bullet-point explanations, one "extremely slow" example with no measured numbers, a comparison table | **Added:** a real, measured timing demonstration of the catastrophic-backtracking example (on safely small inputs), a "why" column added to the comparison table, links to Wikipedia/PEP references for DFA, NFA, Big-O notation, and the pumping lemma, and a Mermaid side-by-side diagram of how a DFA and an NFA each process a pattern. **Fixed:** one inconsistent bullet ("They are Lazy quantifiers" reworded to match its sibling bullets' "cannot support" phrasing). |
| Mini Project: `a*b` matcher | Step-by-step task description, a flowchart image + table, a combined script, a line-by-line explanation | **Added:** a Mermaid version of the flowchart (redrawn from the table, alongside the original image), a short "why build this by hand?" motivating paragraph. **Kept verbatim:** every task/hint/Dos-and-Don'ts line, and the script's logic (already correct and already fully verified). |
| Extension Tasks 1–4 + Optional Challenge | Four extension tasks and a "hard mode" challenge, each with a script, output, and line-by-line explanation | **Added:** a Mermaid diagram showing how the five stages (Tasks 1–4 plus the Optional Challenge) build on each other, `# Step N` comments added throughout every script (most already had partial comments), light copy-editing of the line-by-line explanations for consistent formatting. **Kept:** every task description, hint, checklist, and the conceptual Q&A in Task 4 verbatim — all five scripts were re-verified and produced output identical to the original. |
| "Missing classes" investigation | The investigation, its questions, a cheat sheet, and a claim that the internal C class is named `_sre.SRE_Pattern` / `_sre.SRE_Match` | **Fixed:** the specific internal-naming claim — see the corrections table. **Added:** a verified table of what is actually observable on Python 3.12, an extra follow-up question suggesting readers try `hasattr(_sre, 'Pattern')` themselves, and a link to the CPython C source file involved. **Kept verbatim:** the four numbered "Questions for the Student," the framing of the exercise, and its one-sentence summary. |
...
| # | Location | What the original said | What was verified to actually happen | Fix applied |
|---|---|---|---|---|
| 1 | Script 5 (Pattern instance methods) | The last two `print()` lines use `r'\\d+'` (a double backslash), with comments claiming the output reads `Sub \d+ With ...` | Running it prints `Sub \\d+ With ...` (with two backslashes) instead | Changed to `r'\d+'`, matching script 4's convention and the intended, single-backslash output |
| 2 | Script 7 (flags) | A prose sentence and a second `import re` were left inside the first Python code fence, and the text claimed `p.flags` prints `10` | The stray prose makes the block invalid Python; and `p.flags` actually prints `42`, because Python 3 automatically adds the `re.UNICODE` flag (32) to patterns compiled from `str` | Split into two separate, valid code blocks; corrected the printed value to `42` with an explanation of the automatic `re.UNICODE` flag |
| 3 | Script 11 (alternation) | Claimed a capturing group `(cat\|dog\|rat)` makes `findall()` return `[('cat',), ('dog',), ('rat',)]` (tuples) | With exactly **one** capturing group, `findall()` returns a plain list of strings, identical to the non-capturing version — Python's own documentation confirms tuples only appear with two or more groups | Corrected the claimed output to the verified list of strings, and explained the one-group-vs-many-groups rule |
| 4 | Script 13 (file extensions) | Claimed `m1.group(1)` is `"docx"` and `m2.group(1)` / `m3.group(1)` are `"notes"` | `re.search()` returns the **first** (leftmost) match, and `"report.pdf"` appears before `"notes.docx"` in the sentence, so the true results are `"pdf"` and `"report"` | Corrected the claimed outputs, and added a note explaining why `re.search()` found `report.pdf`, not `notes.docx` |
| 5 | Q1 code snippet | `import re print(re.match(...))` printed as one run-together line | This is invalid Python syntax as written (two statements with no separator) | Reformatted onto separate lines |
| 6 | Q10 code snippet | `text = "<tag>content</tag>" re.search(...)` printed as one run-together line, with no `import re` and no `print()` around the `.group()` calls | As written, this cannot run (invalid syntax, and no output would print even if it could) | Reformatted onto separate lines, added the missing `import re`, and wrapped both calls in `print()` |
| 7 | Q18 code snippet | `p = re.compile(r"\d+") p.findall(...)` printed as one run-together line, with no `import re` or `print()` | Same class of problem as items 5 and 6 | Reformatted onto separate lines, added the missing `import re` and `print()` |
| 8 | Q20 (`re.VERBOSE` example) | The whole verbose pattern was written on a single line: `re.compile(r""" \d+     # digits \s*     # optional space """, re.VERBOSE)` | Because everything sits on one line, the first `#` turns *everything after it* — including `\s*` and the second `#` — into one long VERBOSE-mode comment. The pattern Python actually compiles only ever matches `\d+`; the "optional space" part silently never applies | Rewrote the example as genuinely multi-line, and added a note explaining exactly why the one-line version was quietly broken |
| 9 | Password validation test list | `"Ab1@xy!"` was listed among the "invalid" passwords, with a comment second-guessing its own length ("Too long (7 but extra special allowed? no length OK — keep for discussion)") | At 7 characters, with one of each required character type, `"Ab1@xy!"` satisfies every rule and is correctly reported as `GOOD` by the script | Replaced the uncertain comment with an explanation confirming `"Ab1@xy!"` is genuinely a valid password under the stated rules, and added a note distinguishing it from the very similar-looking (but genuinely invalid) `"Ab1@xy "` two lines below it |
| 10 | DFA/NFA comparison bullets | "They are Lazy (non-greedy) quantifiers" listed as a "feature NOT supported by DFA," inconsistent in phrasing with its two sibling bullets | Purely a wording inconsistency, not a factual error | Reworded to "They cannot support lazy (non-greedy) quantifiers in the general case," matching the phrasing of the two bullets above it |
| 11 | "Missing classes" investigation | States the internal C class is named `_sre.SRE_Pattern` / `_sre.SRE_Match`, and that the "hack" renames this cryptic name to the friendly `re.Pattern` / `re.Match` | On Python 3.12, `type(re.compile('')).__module__` is already `'re'` and `.__qualname__` is already `'Pattern'` — the C type is already named and displayed as `re.Pattern` before the "hack" lines even run. What *is* still true and verified: `_sre` never exposes this class under any attribute name (`hasattr(_sre, 'Pattern')` is `False`), so `type(...)` extraction remains the only way to obtain a usable reference to it | Corrected the specific internal-name claim, added a verified before/after table, while keeping the core "you can't import it, so extract it via `type()`" lesson intact (that part was, and remains, correct) |
| 12 | TOC link for the primality mini-project | Pointed to `#mini-project-simulating-a-regular-expression-matcher-in-python` — the anchor for the *other*, later mini-project (the `a*b` matcher), not the primality section | Following the original link lands on the wrong section | Corrected to `#using-regular-expression-re-to-check-for-primality`, the primality section's own anchor |
| 13 | TOC link for "The Complete Python script..." | Pointed to an anchor that GitHub never generates, because the target text lives inside a `<summary>` tag, not a real Markdown heading | Following the original link does nothing, since GitHub does not create clickable anchors for `<summary>` text | Added an explicit `<a id="...">` anchor immediately above that `<summary>`, so the link now genuinely works |
