...
| --- | --- | --- | --- | --- |
| 1 | Page title | Page began with the level-2 heading "Why Do We Need Violin Plots?" | Added | Level-1 page title "Violin Plots in Matplotlib: Seeing the Shape of Your Data" |
| 2 | Introduction | Not present | Added | Introduction describing the contents of the page and its link to Python and the Matplotlib chapter |
| 3 | Table of Contents | Not present | Added | Nested list of links to all level 1 to 3 headings (the page has no level-4 headings) |
| 4 | "Back to the Table of Contents" links | Not present | Added | Link placed at the end of every section and sub-section |
| 5 | The Problem with Box Plots | One blockquote | Added | Blockquote kept; explanation of Q1, Q3 and median with link; note that Matplotlib whiskers stop at 1.5 × IQR; paragraph on why shape matters |
| 6 | Real Question A Beginner Asks | Question, answer and solution run together inside one blockquote line; heading ended with a colon | Modified | Question kept word for word in its own blockquote; answer and solution placed on separate lines with a short reason and a link to the Bimodal section |
| 7 | What Each Plot Type Reveals table | Histogram "Hides" column listed limitations ("Takes more space, harder to compare multiple groups") rather than hidden information | Corrected | New "Limitations" column added; "Hides" column now lists what is actually hidden; definitions of peak, mode and cluster added |
| 8 | Simple Definition | Said a violin plot "combines a box plot with a density curve" | Modified | Definition kept; table added explaining that Matplotlib draws lines (not a box) that must be switched on, while Seaborn draws an inner box; explanation of density curve, kernel density estimation and bandwidth, with links |
| 9 | Why Is It Called a "Violin" Plot? | Diamond-shaped text drawing inside a `python` code block | Modified | Replaced with a violin-shaped drawing in a `text` block, with labels for narrow and wide sections and the direction of values |
| 10 | Concept Box: Understanding Width | Several ideas run together in one bullet | Modified | Split into separate bullets; added how to read the value from the axis, and a note that Matplotlib scales every violin to the same maximum width |
| 11 | Component Breakdown table | Median "Thick red line", mean "Green triangle marker", Q1 and Q3 "Horizontal lines marking box edges" presented as what a violin plot shows | Corrected | In Matplotlib all lines are blue by default, the mean is a line (not a triangle) and quartiles appear only with `quantiles`; table rebuilt with a "How to Switch It On" column and a row for minimum and maximum; note added that the green triangle belongs to box plots |
| 12 | From Histogram to Violin Plot headings | "From Histogram to Violin Plot: The Transformation" and "Step-by-Step Process" both at level 3 | Modified | Section heading moved to level 2 so that its sub-headings nest under it; image link kept unchanged |
| 13 | What Happens at Each Step? | Six-row table | Added | Numbered Mermaid flowchart of the six steps; note that Matplotlib builds the curve directly with kernel density estimation; step 4 wording clarified |
| 14 | Comparison heading | "Comparison: Histogram vs. Box Plot vs. Violin Plot" at level 3 | Modified | Moved to level 2 |
| 15 | Feature Comparison Table | Quartiles "Yes" for violin; outliers "Sometimes" | Modified | Quartiles note "when `quantiles` is set" added; outliers row explains they are not marked separately |
| 16 | When Each Plot Shines | Several bullets run together ("Exact frequency counts To see every detail of shape .", "To identify outliers clearly Space-efficient display", "To see skewness clearly Better than box plot...") | Corrected | Split into separate, complete bullets; numbered Mermaid decision flowchart and explanation of strip plot added |
| 17 | Typical Violin Plot Shapes | Heading at level 3; the four shapes at the same level; star emoji in the bimodal heading | Modified | Section heading moved to level 2; emoji removed; note on upright orientation, mean versus median for each shape, real-life examples and links added |
| 18 | Script for the four shapes | Not present | Added | Script drawing all four shapes side by side, with printed means and medians, a new image and a table explaining the output |
| 19 | Basic Violin Plot Script heading | "Basic Violin Plot Script" followed by a level-2 heading "Script" | Modified | "Script" heading replaced by "Basic Script Step by Step", "Basic Script: Complete Script" and "Output of the Basic Script" |
| 20 | Basic script comments on colors | "Show mean as green triangle... Green triangle is default marker for mean in violin plot"; "Red line is default marker for median in violin plot" | Corrected | Matplotlib draws both as blue lines by default; the script now colors the median red and the mean green itself |
| 21 | Basic script code | Six steps; no printed output; no quartiles | Modified | Eight steps with printed output after each; `quantiles=[0.25, 0.75]` added; return value stored as `parts` and each part colored; x tick label and legend added; complete script and full output given; image redrawn (same file name) |
| 22 | Basic script terms and flow | Not present | Added | Table of terms (random seed, standard deviation, percentile, legend) with links, and a numbered Mermaid flowchart of the script |
| 23 | Why This Script Is Written This Way | Point 5 said a green triangle marks the mean and described "narrow tails at 25, 30, and 105" | Corrected | Point 5 updated to green line for mean, red line for median, dashed lines for quartiles, and thin tails reaching 25 and 105; points 1 to 4 kept |
| 24 | Reading the basic plot | Not present | Added | Five-step guide to reading the plot using the actual printed values; notes on the "layers" and on building the legend |
| 25 | Follow-up questions on the basic script | Not present | Added | Three questions with answers, including a short script comparing statistics with and without outliers |
| 26 | Bimodal scenario text | "the scores of the two classes ... is combined ... is given"; double full stop | Corrected | Grammar corrected ("are combined", "are given"); extra full stop removed |
| 27 | Bimodal comparison script | Five steps; statistics printed after `plt.show()`; `vert=False` used for box and violin | Modified | Eight steps with output after each; class means and counts in three score bands printed; summary printed before `plt.show()` so it appears without closing the window; `orientation="horizontal"` used with a note on `vert=False` for older versions; `sharex=True` added so the three panels line up; median and mean lines in the violin colored; image redrawn (same file name) |
| 28 | Bimodal script flow | Not present | Added | Numbered Mermaid flowchart of the script |
| 29 | What to Observe table | Box plot row said "Data ranges from X to Y" | Modified | Actual values added (Q1 48.1, Q3 80.2, range about 34 to 93); step-by-step explanation of why the box plot is fooled, using the printed counts |
| 30 | Why This Matters | Bullets run together ("You miss that Class A struggled You miss that Class B excelled."; "violin    plots") | Corrected | Split into separate bullets; extra spaces removed |
| 31 | Follow-up questions on the bimodal script | Not present | Added | Three questions with answers, including how too much smoothing can hide peaks |
| 32 | Key Options Table | `widths` default given as 0.8; `points` described as "Smoothness of curve"; `showextrema` described as "Show min/max points"; no default column | Corrected | Default column added; `widths` default corrected to 0.5; `points` explained as the number of points in the outline; `showextrema` described as lines; rows added for `quantiles`, `orientation`, `positions` and `bw_method` (the real smoothness control) |
| 33 | Parts returned by violinplot() | Not present | Added | Table of the dictionary keys returned by `violinplot()` with examples |
| 34 | Good Use Cases | "Analyzing survey responses"; "multiple regimes" and "multiple modes" not explained | Modified | Survey row limited to numeric answers, with a caution about rating scales; "regimes" and "modes" explained |
| 35 | When NOT to Use Violin Plots | Table only | Added | Explanation of why small datasets mislead |
| 36 | Further Challenges | Code block of five challenges | Kept | Challenge text kept unchanged; heading capitalized |
| 37 | Challenge solutions | Not present | Added | Worked solution for each challenge with step comments, printed output and explanation; new images for Challenges 1, 2, 3 and 5; note that Challenge 5's hint about `points` is only partly right and that `bw_method` controls smoothness |
| 38 | Summary and Glossary | Not present | Added | Summary of key points and a glossary of technical terms with links |
| 39 | Images | Three images: `ch15-matplotlib-violin-steps-in-making.png`, `ch15-matplotlib-violin1.png`, `ch15-matplotlib-violin-bimodal.png` | Kept, redrawn and added | `ch15-matplotlib-violin-steps-in-making.png` kept unchanged; `ch15-matplotlib-violin1.png` and `ch15-matplotlib-violin-bimodal.png` redrawn to match the improved scripts; five new images added: `ch15-matplotlib-violin-shapes.png`, `ch15-matplotlib-violin-challenge1-trimodal.png`, `ch15-matplotlib-violin-challenge2-horizontal.png`, `ch15-matplotlib-violin-challenge3-groups.png`, `ch15-matplotlib-violin-challenge5-smoothness.png` |
| 40 | All outputs | No printed output shown for either script | Added | Every output on the page was produced by running the scripts (Matplotlib 3.10, NumPy 2) |
