...
| --- | --- | --- | --- | --- |
| 1 | Page title | "Understanding Coordinate Data and Grid/Matrix Data in Matplotlib" | Modified | Renamed "Coordinate Data and Grid (Matrix) Data in Matplotlib" |
| 2 | Introduction | Not present | Added | Introduction explaining what the page covers and how it relates to Python and the Matplotlib chapter |
| 3 | Table of Contents | Not present | Added | Nested list of links to all level 1 to 4 headings; level-5 headings (the (a) and (b) parts of Tasks 4 and 5) left out to avoid clutter |
| 4 | "Back to the Table of Contents" links | Not present | Added | Link placed at the end of every level 2, 3 and 4 section |
| 5 | Problem Statement, Task Requirements, Expected Learning Outcomes | Question and task text | Kept | Wording kept unchanged; horizontal rules removed; Q1 to Q3 and Tasks 1 to 6 changed from level 3 to level 4 headings so they sit under "Part A" and "Part B" in the Table of Contents; the (a) and (b) parts moved to level 5 |
| 6 | Before You Start | Not present | Added | Table of libraries with links, install command, table of basic terms (array, matrix, index, shape, DataFrame, color bar), notes on version differences and on the images |
| 7 | Heading structure of the solution | "# Solution" and "# Full Combined Model Script" at level 1; "## 1. Explaining...", "### 1. Coordinate Data", "## 2. Grid or Matrix Data"; "## Answer2.", "## Answer3."; `plot()` at level 3 but `scatter()`, `imshow()`, `contour()` at level 2 | Modified | Arranged as "Solution to Part A" and "Solution to Part B" (level 2), one level-3 heading per answer or task, and level-4 headings inside; small headings such as "Meaning", "Purpose", "Best Use", "Simplified Syntax", "Example", "Output" and "Explanation" turned into bold labels |
| 8 | Duplicate headings | "Common Beginner Error" (twice), "Important Note" (twice), "Common Error Example" (three times), "Why This Task is Important" (twice) | Modified | Each given a unique, descriptive name so every link works |
| 9 | Q1 coordinate general form and example | Shown as Python code: `(1, 18)(2, 21)(3, 25)`, which is not valid Python | Corrected | Shown as plain text with spaces between the points |
| 10 | Q1 coordinate data | No script | Added | Explanation of storing coordinate data as two lists, a four-step script with `zip()`, output and graph |
| 11 | Q1 grid example | Python list of lists with uneven formatting | Modified | Re-indented; shape (3, 3) and an example of locating a value by row and column added |
| 12 | Q1 temperature grid script | Script without axis labels, city or month names, or color bar; output was only an image | Modified | Split into seven steps with printed matrix, shape and a sample value; axis labels, city and month tick labels, color bar and values written in cells added; complete script, output and new image given |
| 13 | Q1 grid data | No real-life example list for grid data | Added | Five real-life examples; link for heat map; sentence explaining the brightest and darkest cells |
| 14 | Comparison between coordinate and grid data | Four rows | Added | Two more rows (values needed to locate a reading, how it is stored in Python) |
| 15 | Flowchart: Types of Plot Data | Placeholder "XXX" | Added | Mermaid flowchart for deciding the data type and the plotting method |
| 16 | Important Note on two variables | Blockquote | Modified | Blockquote text kept; heading made unique; explanation added on counting labels to tell the two kinds of data apart |
| 17 | Q2 headings | "Answer2. Solution to: Explain the Purpose of the Following Matplotlib Methods" | Modified | Renamed "Answer to Q2: Purpose of plot(), scatter(), imshow() and contour()" |
| 18 | Q2 "kind of data accepted" | Not stated for each method, although the question asks for it | Added | "Kind of Data Accepted" paragraph for each of the four methods; links to the Matplotlib documentation |
| 19 | Q2 `plot()` example | Line `graphplt.plot(x, y)` (comment and code run together, so the script fails) | Corrected | Split into a comment line and `plt.plot(x, y)`; title, labels, a print statement, step comments, output and image added |
| 20 | Q2 `scatter()` example | Script without labels or output | Modified | Step comments, print statement, title, labels, output, image and a one-line reading of the graph added |
| 21 | Q2 `imshow()` example | Script without output | Modified | Step comments, shapes before and after `reshape`, printed matrix, color bar, title, output, image and explanation added |
| 22 | Q2 `contour()` example | Code block with no language; line `plotplt.contour(X, Y, Z)` (comment and code run together) | Corrected | Split into a comment and `plt.contour(X, Y, Z)`; `python` language added; explanation of `meshgrid`, shape prints, contour labels, title, axis labels, output and image added; `contourf()` introduced |
| 23 | Comparison of Plotting Methods | Three columns | Added | "Data Accepted" column |
| 24 | Common beginner error with `imshow()` | Said a 1-D list "may cause an error" | Corrected | States that it causes an error; script added showing the actual `TypeError` message and the fix with `reshape` |
| 25 | Q3 heading | "Answer3. Solution to: Explain the Role of Matrix Slicing in NumPy" | Modified | Renamed "Answer to Q3: Role of Matrix Slicing in NumPy"; link to NumPy indexing added |
| 26 | Q3 example matrix output | Printed matrix with broken line alignment, in a `python` block | Corrected | Replaced with the real output in a `text` block; table with row and column index numbers added; links for `arange` and `reshape` |
| 27 | Q3 general syntax | Syntax split over two lines | Modified | Written on one line; "If left out" column added to the table of terms; five-step method for writing a slice added |
| 28 | Q3 Examples 1 to 3 | Example 1 code block had no language; outputs had broken alignment; Example 2 explanation split awkwardly | Corrected | Language added; real outputs used; Example 2 explanation tidied; highlighted table added for Example 3; the four blocks also given as one script |
| 29 | Flowchart: Matrix Slicing | Placeholder "XXX" | Added | Mermaid flowchart of the steps for writing a slice |
| 30 | Small Concept Script | Code block with no language; no output | Modified | Language, step comments, shape prints and real output added |
| 31 | Index error example | Said "IndexError may occur" | Corrected | States that it occurs; script added showing the actual message, negative indexing, and the fact that an over-long slice does not raise an error |
| 32 | Note on ending index | "`A[1:4]` means: `1, 2, 3`" | Modified | Clarified as rows 1, 2, 3; quick check `row_end - row_start` added |
| 33 | Follow-up questions on slicing | Not present | Added | Three questions with answers and a script that checks them |
| 34 | Two comparison tables of the four methods | Placed at the end of the Q3 answer with no parent heading | Modified | Placed under "Comparison of the Four Methods"; content kept; note explaining "Limited" use of color added |
| 35 | Part B overview | Heading "Part B — Programming Model Solution" | Modified | Renamed "Solution to Part B"; Mermaid flowchart of the six tasks added; heading "Part B — Programming Model Solution (Continued)" removed |
| 36 | Task 1 script | One block; printed first rows and column names | Modified | Five steps with outputs; shape, data types and value ranges printed; complete script and full output added |
| 37 | Task 1 column table | Three rows, two columns | Added | Data type and example columns; units (thousands of passengers); notes on 144 rows, the row index, the category type and long form; link on time series |
| 38 | Task 2 "Why Conversion Is Needed" | "`imshow()contour()`" run together | Corrected | Written as "`imshow()` and `contour()`" |
| 39 | Task 2 script | Very long inline comments; printed matrix only | Modified | Inline comments shortened (full explanation kept in the block comment); five steps with outputs; shape and one value using `.loc` printed; Mermaid diagram and before-and-after table explaining `pivot()` added; notes on reading the output and on month order added |
| 40 | Task 3 script | Comment said the NumPy conversion is needed because `imshow()` requires it | Corrected | Explained that the conversion is needed for position-based `[rows, columns]` slicing; six steps with outputs, including a list of row index and year |
| 41 | Task 3 explanation | Short bullet list | Added | Row index table showing the selected years; table comparing NumPy slicing with pandas `.iloc` and `.loc` |
| 42 | Task 3 common error | Said it "may produce an error" | Corrected | States the actual error in pandas 3.0 and gives two ways to fix it |
| 43 | Task 4 example table | Jan 196, Feb 196, Mar 236 shown for the selected year; these are the 1953 values | Corrected | Replaced with the 1955 values (242, 233, 267) |
| 44 | Task 4 scripts | Three separate full scripts, each reloading the data; long inline comments | Modified | Written as three continuing steps of one script, followed by a complete script and full output; `marker="o"` added to the line plot; busiest and quietest months printed |
| 45 | Task 4 Step 1 output | Showed only May to December, with no header row | Corrected | Full real output with all 12 months and header |
| 46 | Task 4 Step 2 output | Repeated the printed table instead of showing the graph | Corrected | Real printed output and the line plot image given |
| 47 | Task 4 explanation and comparison | Short explanation; comparison table only | Added | Actual seasonal values from 1955; note on text labels on the x-axis; five-point comparison of the two graphs |
| 48 | Task 4 common error | Said missing quotes cause `NameError` | Modified | Kept, and added the correct form and what happens if a variable called `month` already exists |
| 49 | Task 5 Step 1 comment | Said rows 3 to 9 "correspond to the years 1956 to 1962" (repeated in the `imshow()` step) | Corrected | Changed to 1952 to 1958 |
| 50 | Task 5 `imshow()` and `contourf()` scripts | Y-axis labelled "Years" but showing 0 to 6; x-axis showing 0 to 11; long repeated comments | Corrected | Month names and years added as tick labels; color bars labelled; written as continuing steps with outputs, followed by a complete script; contour levels printed |
| 51 | imshow() and contourf() comparison | Four-row table | Added | Three more rows; five-point comparison, including the opposite direction of the year axis, `origin="lower"`, and a caution about smooth curves between months |
| 52 | Task 5 common error | Said a list "may produce an error" | Corrected | States the actual error message and adds the `contourf()` message |
| 53 | Task 6 answers | "`plot()scatter()`" run together; Q3 heading used `contourf()` while the task asks about `contour()` | Corrected | Written as "`plot()` and `scatter()`"; heading matches the task wording, with `contourf()` mentioned in the answer |
| 54 | Task 6 content | Short answers | Added | Order of difficulty for all methods, advice on choosing between `imshow()` and `contour()`, reasons based on the actual graphs, and three follow-up discussion questions |
| 55 | Full Combined Model Script | Covered only Tasks 4 and 5; no prints; matrix plots without axis labels or tick labels; very long inline comments | Modified | Covers Tasks 1 to 5 with step comments and print statements; axis labels, tick labels, labelled color bars and `tight_layout()` added; output and both images given; explanation of `subplots` and the `ax` style added |
| 56 | Output blocks | Printed outputs placed in `python` blocks | Modified | Placed in `text` blocks |
| 57 | Glossary | Not present | Added | Glossary of technical terms with links |
| 58 | Images | Four images: `ch15-temperature-grid.png`, `ch15-passenger-distribution.png`, `ch15-passenger-imshow.png`, `ch15-contourf.png` | Modified and added | The four images redrawn to match the improved scripts (same file names); nine new images added: `ch15-coordinate-example.png`, `ch15-plot-example.png`, `ch15-scatter-example.png`, `ch15-imshow-example.png`, `ch15-contour-example.png`, `ch15-imshow-error-fixed.png`, `ch15-passenger-line-plot.png`, `ch15-combined-coordinate-plots.png`, `ch15-combined-matrix-plots.png` |
| 59 | All outputs | Several outputs missing, truncated or misaligned | Corrected | Every output on the page was produced by running the scripts (Matplotlib 3.10, NumPy 2, pandas 3.0, Seaborn 0.13.2) |
