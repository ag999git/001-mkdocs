...
| --- | --- | --- | --- | --- |
| 1 | Page title | Page began with the level-2 heading "Data Types and Data Visualization Before Learning Matplotlib" | Modified | Replaced with a level-1 page title "Data Types and Choosing the Right Plot: The Foundation for Matplotlib" |
| 2 | Introduction | Not present | Added | Four-paragraph introduction explaining what the page contains and how it relates to Python and the Matplotlib chapter |
| 3 | Table of Contents | Not present | Added | Nested list of links to all level 1 to 4 headings, with level-4 headings placed under their level-3 parent |
| 4 | "Back to the Table of Contents" links | Not present | Added | Link placed at the end of every section and sub-section |
| 5 | Objective and Research Question | Placed directly under the old page heading | Modified (text kept word for word) | Moved under a new heading "Project Overview"; added a note mapping the two parts of the research question to the sections that answer them, and three follow-up questions |
| 6 | Chapter Introduction | Four example bullets and a five-point list | Modified | Wording lightly polished, key plot names put in bold, and a small table of basic terms (variable, observation, dataset, distribution) added |
| 7 | Types of Data | Summary table only | Added | Explanation of the two classification systems (levels of measurement, and discrete versus continuous), qualitative versus quantitative, link to Wikipedia, and a Mermaid classification chart |
| 8 | Sections 1 to 6 (Nominal to Continuous) | Level-2 headings separated by horizontal rules | Modified | Changed to level-3 headings under "Types of Data" so the Table of Contents nests correctly; horizontal rules removed |
| 9 | Nominal Data | Short definition, examples and plot lists | Added | Meaning of "nominal", the shuffle test, and the reason a line graph is unsuitable |
| 10 | Ordinal Data | Short definition and examples | Added | Example scale for satisfaction levels, explanation of unequal gaps, why the median is preferred, and what "ordered bar chart" means |
| 11 | Interval Data | One example only; no suitable plots listed | Added | Second example (calendar years), explanation of an arbitrary zero with the 20 °C and 10 °C example, and a list of suitable plots |
| 12 | Ratio Data | Short definition | Added | Explanation of a true zero with a weight example |
| 13 | Discrete and Continuous Data | Short definitions | Added | Further explanation, and links for "stem plot" and "density plot" |
| 14 | Comparison of properties table | "Scaled, Numeric (no zero)" and "Scaled, Numeric (with zero)" | Modified | Changed to "no true zero" and "with true zero"; added a note on how to read the table and definitions with links for mode, median and mean |
| 15 | Further differentiation table | Interval mistake read "Interpreting ratios meaningfully"; Continuous Data row had empty "Suitable Plots" and "Common Mistake" cells | Modified | Interval mistake reworded with an example; empty cells filled in; notes added on "Limited" arithmetic and "Usually Yes" |
| 16 | Quick Test for identifying data type | Not present | Added | Mermaid flowchart, warning about numbers used as labels, and a worked table of six variables |
| 17 | Plot types table | Heading "Plot types"; several blank lines | Modified | Sub-heading "Overview of Common Plot Types" added; a few cells clarified (Scatter Plot limitation; Pie Chart and Box Plot "Unsuitable For"; Box Plot "Best Suitable For" and limitation; Line Plot, Bar Chart, Histogram and Area Plot observations); definitions with links added below the table |
| 18 | Additional Comparison Table and Quick Plot Selection Guide | Present | Kept | Content kept unchanged; one introductory sentence added to the comparison table |
| 19 | Choosing a Plot Step by Step | Not present | Added | Five-step method, Mermaid decision flowchart, and a table linking combinations of data types to plots |
| 20 | Line Plot vs Bar Chart | Two bullets and two examples | Modified | Bullets and examples expanded with reasons |
| 21 | Histogram vs Bar Chart table | Three rows | Added | Three more rows (x-axis, order of bars, bar width) |
| 22 | Scatter Plot Importance | List of uses and patterns | Added | Short meaning for each pattern, link for machine learning, and a note that correlation is not causation |
| 23 | Common Visualization Mistakes | Unnumbered level-3 heading | Modified | Numbered as note 4; table kept unchanged |
| 24 | What Happens When the Wrong Plot Is Chosen | Not present | Added | Table of seven wrong choices with their effects and better choices, and a summary of three kinds of harm |
| 25 | Before You Run the Scripts | Not present | Added | Libraries needed, installation command, internet note, DataFrame and hex color explanations, note on version differences in output |
| 26 | Script 1 introduction | Said the dataset contains "all four major data types"; listed `total_bill`, `tip`, `size` as "Interval/Ratio"; used a LaTeX arrow `$\rightarrow$` | Corrected | Changed to "several data types"; the three columns classified as Ratio (the dataset has no interval column); plain arrow used |
| 27 | Script 1 dataset details | Not present | Added | Table of all seven columns with data types, notes on why `day` is treated as nominal and `time` as ordinal, table of the four plots, and a Mermaid flowchart of the steps |
| 28 | Script 1 code | One block with numbered comments 1 to 3 | Modified | Split into eight steps with "# Step" comments, each shown separately with its output, followed by the complete script and full output; `print()` statements added; comments rewritten in simpler words |
| 29 | Script 1 comment on `observed=False` | Said it preserves the order of categories "as they appear in the dataset" | Corrected | Explained that the order comes from the category column itself and that `observed=False` includes every category |
| 30 | Script 1 `hist()` call | Return values not used | Modified | Return values stored so the bin counts and bin width can be printed |
| 31 | Script 1 explanation and follow-up questions | Not present | Added | What each plot shows (with the real values), table of key code with links, notes on `observed=False` and saving before showing, and three follow-up questions with answers |
| 32 | Image headings | "The resulting plot is as follows:-", "Resulting plot is shown below", "The ouptut plot is as follows" | Modified | Renamed to "The Resulting Plot of Script 1" (and 2, 3, 4) to keep headings unique; spelling of "output" corrected; image links kept unchanged |
| 33 | Box plot heading | "Case Study: Analyzing Continuous Groups with Box Plots" (level 3, no script number) | Modified | Renamed "Script 2: Case Study - Analyzing Continuous Groups with Box Plots" at level 2, with the original introduction placed under "Explanatory Note Before Script 2" |
| 34 | Script 2 introduction | Two paragraphs | Modified | Wording lightly simplified; link to the Palmer Penguins dataset and a table of its columns and data types added |
| 35 | How to Read a Box Plot | Not present | Added | Mermaid diagram and table of the parts of a box plot, with links |
| 36 | Script 2 code | Five steps | Modified | Expanded to seven steps; missing-value counts before and after cleaning printed; summary statistics and 1.5 × IQR outlier calculation added; step outputs, complete script and full output added |
| 37 | Script 2 whiskers and outliers | Whiskers described only as showing "variability"; outliers "mathematically classified" without a rule | Corrected | Stated the 1.5 × IQR rule used by Seaborn and Matplotlib; noted that an outlier is not necessarily an error |
| 38 | Script 2 `.dropna()` explanation | Said skipping it might make Matplotlib "flag errors or create skewed gaps" | Corrected | Explained that 11 rows are removed (only 2 missing body mass), that Seaborn would skip missing masses anyway, and why cleaning is still a good habit |
| 39 | Script 2 `hue` and `legend` explanation | Said a custom palette "requires" `hue` | Corrected | Explained that from Seaborn 0.13 a palette without `hue` gives a deprecation warning, as stated in the warning message |
| 40 | Script 2 results and follow-up questions | Not present | Added | Table of median, quartiles and outliers for each species, and three follow-up questions with step-by-step answers |
| 41 | Script 3 headings | "Explanatory Note of the Script" used twice (before and after the script) | Modified | Renamed "Explanatory Note Before Script 3" and "Explanatory Note After Script 3" so each heading and link is unique |
| 42 | Script 3 introduction | Explanation of histograms and bins | Added | Link to Wikipedia, small worked example with ten marks, and a Mermaid flowchart |
| 43 | Script 3 code | Five steps; very long inline comments repeating the block comments | Modified | Six steps; inline comments shortened (the full explanation kept in the block comment above); summary statistics and counts for each bin printed; step outputs, complete script and full output added |
| 44 | Script 3 "X-Axis Gaps" note | Said gaps would imply ranges where it was "physically impossible to spend money" and called the x-axis a "timeline" | Corrected | Explained that bars touch because bins are adjacent intervals on a number line, and that a gap means an empty bin |
| 45 | Script 3 skewness note | Peak given as "\$13 to \$18"; text addressed to teachers ("your students") | Corrected | Peak stated from the actual counts (tallest bin \$15.80 to \$18.98, 49 bills); mean and median comparison added; text addressed to the student |
| 46 | Script 3 extras | Not present | Added | Mention of `bins="auto"` with link, table of histogram shapes, and three follow-up questions (including a script comparing 3, 15 and 100 bins with its output) |
| 47 | Script 4 introduction | Explanation of scatter plots | Modified | "Patterns, patterns of movement" repetition removed; explanatory and response variables explained with link; correlation coefficient table added |
| 48 | Script 4 code | Five steps; very long inline comments | Modified | Seven steps; correlation coefficient and tip ranges for small and large bills calculated and printed; step outputs, complete script and full output added |
| 49 | Script 4 "Identifying Correlation" note | Said the plot "visually proves" the theory | Corrected | Correlation value 0.68 added; wording changed to "supports", with a reminder that correlation does not prove cause |
| 50 | Script 4 "Alpha" note | Referred to "thousands of data points" and the "customer demographic" | Corrected | Noted that the dataset has 244 rows; described where most points lie in plain words |
| 51 | Script 4 "Variance and Anomalies" note | Tips at a \$10 bill "roughly \$1 to \$3"; at a \$40 bill "from \$2 to \$10" | Corrected | Replaced with values from the data: \$1.25 to \$4.00 for \$8 to \$12 bills, \$2.50 to \$7.58 for \$35 to \$45 bills; the \$10 tip placed on the \$50.81 bill |
| 52 | Script 4 follow-up questions | Not present | Added | Three questions with answers, including a script that colors dots by `smoker` and its output |
| 53 | Summary and Glossary | Not present | Added | Summary of key points and a glossary of technical terms with links |
| 54 | Spelling and formatting | Mixed spellings ("colour" and "color", "Analysing" and "Analyzing"); unescaped dollar signs in text | Modified | One spelling style (color, analyzing) used throughout; dollar signs in running text escaped so GitHub does not treat them as mathematical formulas |
| 55 | All script outputs | Not present | Added | Every output shown was produced by running the scripts (Matplotlib 3.10, Seaborn 0.13.2, pandas 3.0) |
