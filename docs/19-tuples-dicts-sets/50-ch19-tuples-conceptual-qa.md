...
| No. | Location | What was in the original | What was changed | Type of change |
| --- | --- | --- | --- | --- |
| 1 | Top of page | No title and no introduction | Added the title "Tuples in Python: Conceptual Questions and Answers" and an introduction explaining what the page contains and how it links to the chapter and to Python in general | Added |
| 2 | Top of page | No table of contents | Added a nested Table of Contents: 8 parts, with the 20 questions listed under them, plus the Key Terms and Quick Revision Summary sections | Added |
| 3 | After the Table of Contents | No glossary | Added "Key Terms Used on This Page" with simple meanings and links to the official Python documentation | Added |
| 4 | All 20 questions | Questions written as bold paragraphs, one after another | Each question turned into a level-3 heading, grouped under 8 level-2 part headings. The wording of every question is unchanged. Each heading has an HTML anchor (for example `<a id="q1"></a>`) so the links work on both GitHub and GitHub Pages | Modified (format only) |
| 5 | End of every section | No navigation links | Added "Back to the Table of Contents" at the end of every section | Added |
| 6 | Whole page | Technical wording such as "instantiate", "primitive integer", "structural slots", "over-provisioned", "architectural lock" | Rewritten in plain language with short sentences; technical terms are explained or linked | Modified |
| 7 | Q1 | Single paragraph | Plainer explanation; added `tuple()` as a second way to make an empty tuple; added a step list, a numbered Mermaid flowchart, a script with output, a summary table and follow-up questions 1.1 and 1.2 | Modified / Added |
| 8 | Q2 | Single paragraph | Added the meaning of tuple packing, a table of cases where brackets are required, a script with output, advice on which form to use and follow-up question 2.1 | Added |
| 9 | Q3 | "refuses to call an internal slot for item modification. Because mutability structural slots are completely missing..." | Replaced with a plain explanation: tuples have no `__setitem__` method. Added the `del t[0]` case, a step list, a script with output and follow-up questions 3.1 (a tuple holding a list) and 3.2 (rebinding) | Modified / Added |
| 10 | Q3 | Error message written as `TypeError`: 'tuple' object does not support item assignment, with the code formatting split | Full error message shown correctly | Modified |
| 11 | Q4 | Lists described as having `.append()`, `.insert()`, `.extend()`, `.remove()` and `.pop()`, all inside one code span | Each method in its own code span; added the other list-only methods `sort()`, `reverse()`, `clear()` and `copy()`; added a comparison table, a script with output, a note on `AttributeError` and follow-up question 4.1 | Modified / Added |
| 12 | Q5 | "remains unchanged at its original address until it is cleaned up by the garbage collector" | Corrected: the old tuple stays only while some other name refers to it; otherwise CPython usually frees it at once. Added a step list, a numbered Mermaid flowchart, a fuller script (with a `backup` name) and follow-up question 5.1 comparing `+=` for tuples and lists | Modified / Added |
| 13 | Q5 | Script output shown as comments only | Output shown in a separate text block, with a note that `id` numbers differ on every computer | Modified |
| 14 | Q6 | Short paragraph | Added that the slice shares the same item objects (shallow copy), that `t[:]` returns the same tuple in CPython, a slicing table, a script with output and follow-up question 6.1 | Added |
| 15 | Q7 | "tuples outperform lists across three primary architectural metrics" | Corrected to say tuples have a small advantage in some areas, and that the choice should mainly depend on whether the data must change | Modified (correction) |
| 16 | Q7 table | Speed row: lists "slower ... due to pointer indirection and resizing logic"; tuples "highly optimized, direct read access speeds" | Corrected: reading items is about the same speed; the real speed gain is in creating fixed tuples, which are built once and reused. Table expanded to 6 rows with plain wording, including dictionary-key use | Modified (correction) |
| 17 | Q7 | No script | Added a script that measures sizes and times, with real output, and a step-by-step reading of the output; follow-up question 7.1 | Added |
| 18 | Q8 | Error messages split across code formatting | Full error messages shown correctly; added a note on the slightly longer message in Python 3.14 | Modified |
| 19 | Q8 | Two-line example only | Added a numbered Mermaid flowchart, a summary table, a script with output and follow-up questions 8.1 and 8.2 | Added |
| 20 | Q9 | Short explanation | Added step list, table of star positions, rules (only one star; star cannot stand alone), a script with output, a link to PEP 3132 and follow-up question 9.1 | Added |
| 21 | Q10 | Short explanation and example | Added the meaning of "casting", a step list, a table including `keys()` and `items()`, a note on view objects, a script with output and follow-up questions 10.1 and 10.2 | Added |
| 22 | Q11 | Explanation and example | Added `tuple(sorted(t))`, `reverse=True`, `key=`, a plain meaning of "stable sort", a table, a script with output and follow-up question 11.1 | Added |
| 23 | Q12 | Bullet points written with the "•" character | Changed to a normal Markdown list; added a step list, a table, a script with output and follow-up question 12.1 on `any()` and `all()` | Modified / Added |
| 24 | Q13 | "it must be completely immutable and hashable" | Corrected: the requirement is that the key is hashable; explained what hashable means and how it relates to immutability for built-in types | Modified (correction) |
| 25 | Q13 | Did not say which line raises the error | Clarified that creating `(1, [2, 3])` works and the error comes only when it is used as a key. Added a numbered Mermaid flowchart, a table, a script with output (including a fix), a note on the Python 3.14 message and follow-up questions 13.1 and 13.2 | Modified / Added |
| 26 | Q14 | Short explanation and example | Added a step list, a fuller script with a practical example and `divmod()`, a note on docstrings and follow-up questions 14.1 and 14.2 | Added |
| 27 | Q15 | "unpacks this temporary tuple into the target variable targets" | Wording fixed. Added the traditional three-line swap, a step list, a numbered Mermaid flowchart, a note that CPython may skip building the tuple, a script with output and follow-up question 15.1 | Modified / Added |
| 28 | Q16 | "the `.items()` method generates a sequence of two-element key-value tuples" | Reworded: `.items()` gives the pairs one at a time as tuples (it returns a view object, not a sequence) | Modified (correction) |
| 29 | Q16 | Mermaid chart with un-numbered boxes and an "Automatic Unpacking Engine" box | Replaced with a numbered chart that shows the loop going back for each pair and ending; added a step list, a script with output and follow-up questions 16.1 and 16.2 | Modified / Added |
| 30 | Q17 | "it ensures the target object is a two-element tuple" | Corrected: a pattern such as `(404, msg)` matches any two-item sequence, both tuples and lists, but not strings | Modified (correction) |
| 31 | Q17 | Short example without a default case | Added a step list, a numbered Mermaid flowchart, a fuller script with a guard, a wildcard and a final `case _`, explanations of these terms, documentation links and follow-up question 17.1 | Added |
| 32 | Q18 | Single paragraph | Added a step list, a row-and-column table, a Mermaid flowchart, a script with output (including negative indexes, loops and nested unpacking) and follow-up questions 18.1 and 18.2 | Added |
| 33 | Q19 | Example showing `print(t1 is t2)` with the output `False` for two tuples typed as `(1, 2, 3)` | Corrected: when this code is run from a `.py` file, CPython usually prints `True`, because it stores one shared copy of an identical fixed tuple. The script now builds the second tuple with `tuple([1, 2, 3])` so the result is reliably `False`, and the original example is kept with an explanation | Modified (correction) |
| 34 | Q19 | Short explanation | Added a book analogy, a step list, a "when to use which" table, and follow-up question 19.1 on comparing tuples with `<` and `>` | Added |
| 35 | Q20 | Explanation and example | Added a step list, a numbered Mermaid flowchart, a fuller script with a reusable `remove_at()` function and removal by value, a table of other methods and follow-up questions 20.1 and 20.2 | Added |
| 36 | End of page | No summary | Added a Quick Revision Summary table with links back to each question | Added |
| 37 | All scripts | Outputs shown only as comments, or not shown | Every new script has `# Step` comments and its output shown in a separate text block. All scripts were run on Python 3.10 to 3.13 to confirm the outputs | Added |
| 38 | Questions | Question wording | No question has been changed | Unchanged |
