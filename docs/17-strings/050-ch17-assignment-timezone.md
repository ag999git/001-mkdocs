...
|---|---|---|---|---|
| 1 | Page title | `# Assignment: Beyond The Basics- Timezones, Calendars, And Datetimes` | Title changed to "Beyond the Basics: Timezones, Calendars, and Datetimes in Python" with normal capitalisation and punctuation | Modified |
| 2 | Introduction | One line: "This online guide is about processing temporal data..." | Replaced with a full introduction: why time handling matters, link to the strings chapter (`strftime()` and `strptime()`), what the page contains, and setup notes for Python 3.9+, Windows (`tzdata`) and Google Colab | Modified and expanded |
| 3 | Table of Contents | Not present | Nested Table of Contents added with links to all headings down to `####` level; `#####` sub-headings left out to keep it uncluttered | Added |
| 4 | Navigation | Not present | "Back to the Table of Contents" link added at the end of each section listed in the Table of Contents | Added |
| 5 | Section 1 intro | Short description of the two categories | Added the "photo of a clock with and without a label" analogy and a link to the official Python docs on aware and naive objects | Expanded |
| 6 | Section 1A | "Why it is Unsafe" said Python ignores "daylight saving shifts and regional offsets" | Reworded in simpler sentences; added that `tzinfo` is `None` for naive objects | Modified |
| 7 | Section 1B | Aware objects described in one paragraph | Explained that Python can compare aware objects from different zones correctly | Expanded |
| 8 | Section 1C | Not present | Comparison table of naive vs aware objects; note that mixing naive and aware objects raises `TypeError`; numbered Mermaid decision flowchart | Added |
| 9 | Section 1D | Not present | Key terms table (UTC, timezone, UTC offset, DST, `tzinfo`, `ZoneInfo`, microsecond, immutable, ISO 8601, distributed system) with links | Added |
| 10 | Section 2 heading | "Global Standarization" | Spelling corrected to "Global Standardization" | Corrected |
| 11 | Section 2 text | Region names described as "cryptographic string keys" | Corrected to "plain text labels (keys)"; the names have nothing to do with cryptography | Corrected |
| 12 | Section 2 text | "Python accesses this database natively starting in Python 3.9" | Clarified that on Windows the `tzdata` package must be installed; added links to IANA, the tz database, the list of zones and PEP 615; explained why city names are used | Corrected and expanded |
| 13 | Section 2 | Not present | Table of example zones with January and July offsets; sub-section on why abbreviations such as IST are ambiguous; sub-section on setting up `zoneinfo` with a check script and its output | Added |
| 14 | Section 3 heading | "Practical Implementation & Code Snippets" | "&" replaced by "and"; note added that `now()` outputs differ on each computer | Modified |
| 15 | Script A | Outputs written only as inline comments; three steps numbered 1 to 3 | Step comments renumbered 1 to 6; added hour and minute; added `type()` check; added long-date and 12-hour formats (Step 5); added `strptime()` example (Step 6); outputs moved to a text block | Modified and expanded |
| 16 | Script A | Not present | "Understanding the output" notes and a table of common format codes with a link to the full list | Added |
| 17 | Script B heading | "Geographic Localization with `zoneinfo`" | Backticks removed from heading so that the anchor link works on all platforms | Modified |
| 18 | Script B | Kolkata and UTC times only; outputs as inline comments | Step comments added; `utcoffset()` printed; Step 4 converts to New York with `astimezone()`; Step 5 shows that both describe the same moment; outputs moved to a text block with explanation notes | Modified and expanded |
| 19 | Script C heading | "Generating Calendars via the `calendar` Module" | Changed to "Generating Calendars with the calendar Module" | Modified |
| 20 | Script C | Output shown inside a triple-quoted string within the code | Output moved to a separate text block; added `weekday()`, `day_name` and `isleap()` examples; notes on Monday-first weeks, the blank line after the grid, and `setfirstweekday()` | Modified and expanded |
| 21 | Section 4 intro | "Complete these tasks ... to understand/ implement these concepts." | Reworded in plain language with advice to attempt each task first | Modified |
| 22 | Question 1 task text | Question and hint | Kept unchanged | No change |
| 23 | Question 1 | Not present | "Solution Approach" with numbered logical steps and a numbered Mermaid flowchart | Added |
| 24 | Question 1 script | Comment on the naive `tzinfo` line said "Outputs: Aware Timezone Info: Asia/Kolkata" | Corrected: a naive object's `tzinfo` is `None` | Corrected |
| 25 | Question 1 script | Docstring typo "instantiatea"; no offset shown; no check of original object | Typo removed while rewording the docstring; `utcoffset()` printed for both objects; Step 7 added to prove the original object is unchanged | Corrected and expanded |
| 26 | Question 1 output | Output only in comments; explanation used a different time (06:38:15) from the script comments (08:01:53) | Separate output block added; the same time is now used everywhere | Added and corrected |
| 27 | Question 1 explanation | Headings "Detailed explanation", "Why We Use `datetime.now()`...", "How the `.replace()` Method Operates", "The Visual and Functional Difference in Output" | Headings made unique for linking; content kept and reworded in simpler sentences; comparison with string methods added; before-and-after table added | Modified |
| 28 | Question 1 | Implied that `.replace()` always gives correct Kolkata time | New section explaining that `.replace()` only labels and does not convert, with a Google Colab (UTC clock) example script, output, comparison table and Mermaid flowchart | Added |
| 29 | Question 1 | Not present | Follow-up questions 1.1 to 1.3 with answers, including a script that works on any computer | Added |
| 30 | Question 2 task text | Question | Kept unchanged | No change |
| 31 | Question 2 | Heading "Detailed, step-by-step solution." written as plain text | Replaced by "Solution Approach" with numbered steps and a Mermaid flowchart, followed by the script heading | Modified and added |
| 32 | Question 2 script | Outputs only in comments | Comments simplified; offset and aware check printed; output moved to a separate text block | Modified |
| 33 | Question 2 explanation | Used LaTeX `$+00:00$`; said "Python ... normalizes the current numbers back"; referred to "line 29"; phrase "timezone-aware object engine" | LaTeX replaced by plain code text; process explained in three clear steps; line-number reference removed (it did not match the script); unclear wording removed | Corrected |
| 34 | Question 2 explanation | Mumbai 10:00 AM and Frankfurt 6:30 AM example | Kept, with a note that in summer these are the same moment; added the benefit of keeping the rule in one function | Expanded |
| 35 | Question 2 | Not present | Follow-up questions 2.1 (`utcnow()` is naive and deprecated) and 2.2 (display in IST, ISO 8601 text, `timezone.utc`) with script and output | Added |
| 36 | Question 3 task text | Question | Kept unchanged | No change |
| 37 | Question 3 | Answer started directly with a script | "Short Answer in Plain Words" added before the script | Added |
| 38 | Question 3 script | `import time` was imported but never used | Unused import removed | Deleted |
| 39 | Question 3 script | Steps 1 to 5 across two parts | Renumbered Steps 1 to 8; Step 6 shows both events on the UTC clock; Step 8 shows the size of the error; output moved to a separate text block; link for "latency" added | Modified and expanded |
| 40 | Question 3 | Not present | Timeline table of the two events on three clocks; branching Mermaid flowchart with numbered steps | Added |
| 41 | Risk 1 | Paragraph on out-of-order events | Reworded; concrete New York and Bengaluru example added; ledger and double spending explained in plain words; effects listed | Modified |
| 42 | Risk 2 | Described only the repeated autumn hour; said the job could "crash entirely due to an impossible temporal paradox" | Added the missing spring hour; replaced the vague "temporal paradox" wording with the actual effects (runs twice, skipped, or fails); explained the `fold` attribute with a link to PEP 495 | Corrected and expanded |
| 43 | Risk 3 | Said the 30-minute delay "was miscalculated by the naive system as a massive 3.5-hour delay" | Corrected: the naive result was 4 hours, which is 3 hours 30 minutes too large | Corrected |
| 44 | Question 3 | Not present | Summary table of the three risks; follow-up 3.1 (same-zone subtraction across DST, with script and output) and 3.2 (list of good habits) | Added |
| 45 | End of page | Not present | Section 5 "Summary and Best Practices" table and Section 6 "Further Reading" links | Added |
| 46 | All outputs | Several outputs appeared only as comments | All scripts were run on Python 3.11 and every output block matches the actual result (with fixed sample times for `now()` calls) | Verified |
